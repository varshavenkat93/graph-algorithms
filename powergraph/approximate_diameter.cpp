...
const size_t DUPULICATION_OF_BITMASKS = 1;
...
struct vdata {
  ...
  //Uncomment following to read generated bitstrings by GreenMarl
  /*
	  std::string line;
	  std::ifstream myfile ("bitmask_initial.txt");
	  if (myfile.is_open())
	  {
	  ...
	  else std::cout << "Unable to open bitmask file";
	  
	  */
	  //Uncomment following to generate bitstring in Powergraph and write it to file
	      std::ofstream myfile;
 	      myfile.open ("bitmasks_from_Powergraph.txt",std::fstream::app);
